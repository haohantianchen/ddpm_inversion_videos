...
from ddm_inversion.inversion_utils import  inversion_forward_process, inversion_reverse_process, inversion_reverse_process_two_frames, inversion_reverse_process_two_frames_controlnet
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_yaml",  default="test_5_22_c.yaml")
    ...
    # 新增controlnet的加载
    controlnet_path = "/raid/lurenjie/densepose_weight/controlnet"
    ...
    ldm_stable = StableDiffusionControlNetPipeline.from_pretrained(model_id, controlnet=controlnet).to(device)


    save_path = "/raid/lurenjie/DDPM_inversion/results_5_24/densepose_prog_noise_with_controlnet_with_cf_self-attention"
    ...
    for i in range(len(full_data)):
        ...
        # 新增control_image输入
        control_image_folder = current_image_data['control_img']
        files = sorted(os.listdir(control_image_folder))
        for file in files[1:]:
            control_image_path = os.path.join(control_image_folder, file)
            ...
            if args.mode=="our_inv":
                ...
                w0, _, ws = inversion_reverse_process_two_frames_controlnet(ldm_stable, xT=wts[args.num_diffusion_steps-skip], etas=eta, prompts=[prompt_tar]*2, cfg_scales=[cfg_scale_tar], prog_bar=True, zs=zs[:(args.num_diffusion_steps-skip)], controller=None, control_image=control_image)
                ...
                tgt_save_path = os.path.join(for_save_path_floader, file)
